<script>
    ...
    function handleMoveEnd(targetElement) {
        if (!draggedButton) return;

        if (targetElement) {
            if (targetElement.classList.contains('available-buttons-list') || targetElement.closest('.available-buttons-list')) {
                handleDropOnAvailableButtons({ preventDefault: () => {} });
            } else if (targetElement.classList.contains('add-row-drop-zone') || targetElement.closest('.add-row-drop-zone')) {
                ...
                if (dropTargetBtn) {
                    ...
                    const btnIdx = Array.from(dropTargetRow.children).indexOf(dropTargetBtn);
...
<div class="modal-overlay" transition:slide on:touchmove|preventDefault={handleTouchMove} on:touchend|preventDefault={handleMoveEnd} on:touchcancel|preventDefault={handleMoveEnd}>
        ...
        <form on:submit|preventDefault={save}>
            ...
            <div class="form-group keyboard-builder-section">
                ...
                <div class="keyboard-preview">
                    {#each editableDoc.buttons as row, rowIdx}
                        <div
                            ...
                            on:touchstart|preventDefault={(e) => {
                                // Prevent dragging into empty row on mobile
                                if (row.length === 0) {
                    ...
                    {/each}
                    <div class="add-row-drop-zone" on:dragover={handleDragOver} on:drop={handleDropOnAddRow} on:touchend|preventDefault={(e) => handleMoveEnd(e.target)}>
