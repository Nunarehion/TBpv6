<script>
    ...
    let { doc, columns = [], editingId = null, isMessageRoute = false, selectedCollection } = $props();
...
{#if doc}
    <tr class="table-row">
        ...
        {#each columns as col}
            <td class="table-cell" class:name-text={col.trim() === 'name'} title={doc[col] ?? ''}>
                {#if col.trim() === 'text'}
                    <span>
                ...
                {:else if col === 'buttons'}
                    ...
                    {/if}
                {:else if col === 'images'}
                    {Array.isArray(doc[col]) && doc[col].length > 0
                        ? `${doc[col].length} image(s)`
                        : '-'}
