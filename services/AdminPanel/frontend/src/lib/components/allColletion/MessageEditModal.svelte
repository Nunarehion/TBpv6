    <script>
        ...
        function handleImagesInserted(event) {
            const imageUrls = event.detail;
            if (imageUrls && imageUrls.length > 0) {
                const currentImages = editedDoc.images.split('\n').filter(Boolean);
                const newImagesArray = [...currentImages, ...imageUrls];
                editedDoc.images = newImagesArray.join('\n');
            }
                        ...
                        <div class="form-group">
                            ...
                            {:else if col === 'images'}
                                <textarea
                                    ...
                                    rows="5"
                                ></textarea>
                                <button type="button" on:click={openImagePicker} class="action-button add-image-button">
                                    Добавить картинку
                                </button>
    ...
    {#if showImagePicker} 
        <ImagePickerModal
            on:insertImages={handleImagesInserted}
            on:close={closeImagePicker}
        />
    {/if}
