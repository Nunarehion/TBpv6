<script>
	...
	function handleMoveEnd(targetElement) {
		...
		if (targetElement) {
			if (
				targetElement.classList.contains('available-buttons-list') ||
				targetElement.closest('.available-buttons-list')
			) {
				handleDropOnAvailableButtons({ preventDefault: () => {} });
			} else if (
				targetElement.classList.contains('add-row-drop-zone') ||
				targetElement.closest('.add-row-drop-zone')
			) {
