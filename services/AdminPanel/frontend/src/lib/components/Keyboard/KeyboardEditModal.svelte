<script>
    ...
    let editableDoc = $state({
        ...
        buttons: (doc.buttons && Array.isArray(doc.buttons))
            ? doc.buttons.map(row =>
                (Array.isArray(row)
                    ? row.filter(btn => btn && typeof btn === 'object' && btn.id).map(btn => ({ id: btn.id }))
                    : []
                )
            )
            : [],
        ...
        if (targetElement) {
            if (targetElement.closest('.available-buttons-list')) {
                ...
                if (dropTargetBtn) {
                    ...
                    const rowIdx = Array.from(dropTargetBtnWrapper.parentNode.children).indexOf(dropTargetBtnWrapper);
                            ...
                            {#each availableButtons as button (button.id)}
                                ...
                                >
                                    <div class="available-button-content">
                                        ...
                                        <span class="callback-data-display">{getButtonData(button.id).callback_data}</span>
                                    </div>
                                    <button
                                        type="button"
